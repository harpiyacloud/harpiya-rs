...
pub fn bench(c: &mut criterion::Criterion) {
    ...
    c.bench_function("static_str_into_harpiya_error", |b| {
        b.iter(|| {
            let message = "a string error";
            Error::new(message)
        })
    });
    c.bench_function("string_into_harpiya_error", |b| {
        b.iter(|| {
            let message = String::from("a string error");
            Error::new(message)
        })
    });
    c.bench_function("parse_int_error_into_harpiya_error", |b| {
        b.iter(|| {
            let err = "12.15".parse::<i32>().unwrap_err();
            Error::from(err)
        })
