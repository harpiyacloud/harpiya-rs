...
//! use harpiya_core::{state::State, LazyLock};
//!
...
impl<T> State<T> {
    ...
    /// which can be specified by the environment variable `HARPIYA_APP_CONFIG_FORMAT`.
    /// By default, it reads the config from a local file. If `HARPIYA_APP_CONFIG_URL` is set,
    /// it will fetch the config from the URL instead.
    pub fn load_config(&mut self) {
        let env = self.env.as_str();
        let config_table = if let Ok(config_url) = std::env::var("HARPIYA_APP_CONFIG_URL") {
            ...
            let format = std::env::var("HARPIYA_APP_CONFIG_FORMAT")
...
static DEFAULT_ENV: LazyLock<Env> = LazyLock::new(|| {
    ...
    if let Ok(value) = std::env::var("HARPIYA_APP_ENV") {
        let env: &'static str = value.to_owned().leak();
        return env.into();
    }
