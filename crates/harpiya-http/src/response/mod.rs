...
impl<S: ResponseCode> Response<S> {
        ...
        let result = serde_json::to_value(data)
            ...
            .and_then(|mut value| {
                if let Some(data) = value.as_object_mut() {
                    let mut map = harpiya_core::Map::new();
                    map.append(data);
                    crate::view::render(template_name, map)
                } else {
                    Err(harpiya_core::warn!("invalid template data"))
                }
            });
        match result {
            Ok(content) => {
    ...
    pub fn response_time(&self) -> Duration {
        ...
        #[cfg(feature = "metrics")]
        {
            ...
            metrics::gauge!("harpiya_http_requests_in_flight").decrement(1.0);
            metrics::counter!("harpiya_http_responses_total", &labels).increment(1);
            metrics::histogram!("harpiya_http_requests_duration_seconds", &labels,)
