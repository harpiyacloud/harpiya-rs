...
use harpiya_channel::{CloudEvent, Subscription};
use harpiya_core::{
    application::Agent,
    error::Error,
...
use harpiya_storage::NamedFile;

#[cfg(feature = "auth")]
use harpiya_auth::{AccessKeyId, Authentication, ParseSecurityTokenError, SecurityToken, SessionId};

#[cfg(feature = "auth")]
use harpiya_core::{datetime::DateTime, extension::JsonObjectExt, validation::Validation};
...
pub trait RequestContext {
    ...
    fn new_context(&self) -> Context {
        ...
        #[cfg(feature = "metrics")]
        {
            metrics::gauge!("harpiya_http_requests_in_flight").increment(1.0);
            metrics::counter!(
                "harpiya_http_requests_total",
    ...
    fn parse_query<T: Default + DeserializeOwned>(&self) -> Result<T, Rejection> {
        if let Some(query) = self.get_query_string() {
            ...
            if let Some(timestamp) = self.get_query("timestamp").and_then(|s| s.parse().ok()) {
                ...
                if duration > harpiya_auth::default_time_tolerance() {
    ...
    fn parse_authentication(&self) -> Result<Authentication, Rejection> {
        ...
        if let Some(date) = self.get_header("date") {
            match DateTime::parse_utc_str(date) {
                Ok(date) => {
                    #[cfg(feature = "jwt")]
                    if date.span_between_now() <= harpiya_auth::default_time_tolerance() {
        ...
        K: MACLike,
    {
        ...
        let mut options = harpiya_auth::default_verification_options();
