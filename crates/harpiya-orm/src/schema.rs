...
/// Database schema.
///
/// This trait can be derived by `harpiya_derive::Schema`.
pub trait Schema: 'static + Send + Sync + ModelHooks {
    ...
    async fn create_policy() -> Result<(), Error> {
        if !super::AUTO_MIGRATION.load(Relaxed) {
            return Ok(());
        }
        if cfg!(feature = "orm-postgres") {
            ...
            for col in Self::columns() {
                if col.is_policy() {
                    ...
                    let id_type = if col.type_name() == "Uuid" {
                        "UUID"
                    } else {
                        "INT"
                    };
                    ...
                    let sql = format!("ALTER TABLE {table_name_escaped} ENABLE ROW LEVEL SECURITY;");
                    ...
                    // Isolation policy for the table
                    let isolation_policy = format!(r#"
                        DO $$
                        BEGIN
                            IF NOT EXISTS (
                                SELECT 1 
                                FROM pg_policies 
                                WHERE policyname = '{table_name}_isolation_policy'
                            ) THEN
                                CREATE POLICY {table_name}_isolation_policy
                                ON {table_name_escaped}
                                USING ( {0} = current_setting('{1}.current_tenant')::{2});
                            END IF;
                        END $$;
                        "#, 
                        col.name(), 
                    ...
                    if let Err(err) = pool.execute(&isolation_policy).await {
                        tracing::error!(table_name, "fail to execute `{isolation_policy}`");
                        return Err(err);
                    }

                    let access_role = col.extra().get_str("grand_all");

                    if access_role.is_some() {
                        // Access policy for table
                        let isolation_policy = format!(r#"
                            DO $$
                            BEGIN
                                IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = '{table_name}_access_policy') THEN
                                    CREATE POLICY {table_name}_access_policy ON {table_name_escaped}
                                    USING (
                                        '{0}' = ANY(roles)
                                        OR {1} = current_setting('{2}.current_tenant')::{3}
                                    );
                                END IF;
                            END $$;
                            "#,
