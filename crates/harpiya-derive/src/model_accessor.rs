...
pub(super) fn parse_token_stream(input: DeriveInput) -> TokenStream {
    ...
    fetched_one_queries.push(quote! {
        let mut model = Self::find_by_id::<Map>(id)
            .await?
            .ok_or_else(|| harpiya_core::warn!("404 Not Found: cannot find the model `{}`", id))?;
    ...
    quote! {
        use harpiya_core::{
            ...
            validation::Validation as HarpiyaValidation,
            Map as HarpiyaMap,
        };
        use harpiya_orm::{IntoSqlValue as _, ModelHelper as _};

        impl harpiya_orm::ModelAccessor<#model_primary_key_type> for #name {
            #[inline]
            ...
            fn snapshot(&self) -> HarpiyaMap {
                let mut snapshot = HarpiyaMap::new();
            ...
            async fn check_constraints(&self) -> Result<HarpiyaValidation, HarpiyaError> {
                let mut validation = HarpiyaValidation::new();
            ...
            async fn fetch(query: &Query) -> Result<Vec<HarpiyaMap>, HarpiyaError> {
                let translate_enabled = query.translate_enabled();
                #(#fetched_queries)*
            }

            async fn fetch_by_id(id: &#model_primary_key_type) -> Result<HarpiyaMap, HarpiyaError> {
                #(#fetched_one_queries)*
            }

            async fn random_associations() -> Result<HarpiyaMap, HarpiyaError> {
                let mut associations = HarpiyaMap::new();
                #(#sample_queries)*
                Ok(associations)
            }
        }
