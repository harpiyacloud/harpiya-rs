...
pub(super) fn parse_token_stream(input: DeriveInput) -> TokenStream {
                ...
                if let Some(ident) = field.ident {
                    ...
                    let quote_reference = if let Some(ref model_name) = reference {
                        ...
                        quote! {{
                            ...
                            Some(harpiya_core::model::Reference::new(table_name, column_name))
                        }}
                    } else {
                        quote! { None }
                    };
                    ...
                    let column = quote! {{
                        let mut column = harpiya_core::model::Column::new(#name, #type_name, #not_null, #policy);
    ...
    quote! {
        use harpiya_core::{
            error::Error as HarpiyaError,
            model::{schema, Column},
        };
        use harpiya_orm::{ConnectionPool, Schema};

        static #avro_schema: harpiya_core::LazyLock<schema::Schema> = harpiya_core::LazyLock::new(|| {
        ...
        static #schema_primary_key_column: harpiya_core::LazyLock<Column> =
            harpiya_core::LazyLock::new(|| #primary_key_column);
        static #schema_columns: harpiya_core::LazyLock<[Column; #num_columns]> =
            harpiya_core::LazyLock::new(|| [#(#columns),*]);
        ...
        impl Schema for #name {
            ...
            #[inline]
            fn primary_key_value(&self) -> harpiya_core::JsonValue {
            ...
            async fn acquire_reader() -> Result<&'static ConnectionPool, HarpiyaError> {
                use harpiya_core::{bail, error::Error, warn};
                use harpiya_orm::PoolManager;
                ...
                if let Some(reader) = #schema_reader.get() {
                    ...
                    if let Err(err) = Self::create_policy().await {
                        connection_pool.store_availability(false);
                        bail!(
                            "503 Service Unavailable: fail to acquire reader for the model `{}`: {}",
                            model_name,
            ...
            async fn acquire_writer() -> Result<&'static ConnectionPool, HarpiyaError> {
                use harpiya_core::{bail, error::Error, warn};
                use harpiya_orm::PoolManager;
