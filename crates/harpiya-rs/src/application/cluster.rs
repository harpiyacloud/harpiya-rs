...
use harpiya_http::response::Response;
...
impl Application for Cluster {
    ...
    fn run_with<T: AsyncScheduler + Send + 'static>(self, mut scheduler: T) {
        ...
        runtime.block_on(async {
            #[cfg(feature = "orm")]
            harpiya_orm::GlobalPool::connect_all().await;
            Self::load().await;
        ...
        runtime.block_on(async {
            ...
            let servers = listeners.into_iter().map(|listener| {
                ...
                tracing::warn!(
                    ...
                    app_name,
                    app_version,
                    harpiya_version = env!("CARGO_PKG_VERSION"),
                    "listen on `{addr}`",
                );
                ...
                if is_docs_server {
                    let openapi = harpiya_openapi::openapi();
    ...
    async fn shutdown() {
        let ctrl_c = async {
            ...
            #[cfg(feature = "orm")]
            harpiya_orm::GlobalPool::close_all().await;
        };
        #[cfg(unix)]
        let terminate = async {
