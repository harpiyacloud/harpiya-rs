...
use harpiya_rs::{prelude::*, Request, Result};
use harpiya_model::user::JwtAuthService;
...
pub async fn init_user_session(mut req: Request, next: Next) -> Result<Response> {
    ...
    match User::verify_jwt_claims(&claims).await {
        Ok(verified) => {
            if verified {
                let session = UserSession::<Uuid>::try_from_jwt_claims(claims).extract(&req)?;
...
pub async fn check_admin_role(req: Request, next: Next) -> Result<Response> {
    if req.request_method() == "POST" {
        if let Some(session) = req.get_data::<UserSession<Uuid>>() {
