use crate::{
    controller::{auth, file, stats, user, tenant},
    middleware,
    model::{Tag, User, Tenant},
};
use axum::{
    middleware::from_fn,
    routing::{get, post},
...
use harpiya_rs::DefaultController;
...
pub fn routes() -> Vec<Router> {
    ...
    // Tenant controller.
    let router = Router::new()
        .route("/tenant/new", post(tenant::new))
        .route("/tenant/:id/delete", post(Tenant::soft_delete))
        .route("/tenant/:id/update", post(Tenant::update))
        .route("/tenant/:id/view", get(tenant::view))
        .route("/tenant/list", get(Tenant::list))
        .route("/tenant/import", post(Tenant::import))
        .route("/tenant/export", get(Tenant::export))
        .route("/tenant/stats", get(tenant::stats));
    routes.push(router);
...
pub fn debug_routes() -> Vec<Router> {
    ...
    routes.push(router);
        ...
        // Tenant schema controller.
    let router = Router::new()
        .route("/tenant/schema", get(Tenant::schema))
        .route("/tenant/definition", get(Tenant::definition))
        .route("/tenant/mock", get(Tenant::mock));
    routes.push(router);
